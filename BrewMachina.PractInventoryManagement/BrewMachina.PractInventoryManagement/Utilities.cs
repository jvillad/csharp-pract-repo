...
using BrewMachina.PractInventoryManagement.Domain.OrderManagement;
  ...
  public class Utilities
    {
        ...
        private static void ShowDetailsAndUseProduct()
        {
            ...
            {
                Product? selectedProduct = _inventory.Where(p => p.Id == int.Parse(selectedProductId)).FirstOrDefault();
        ...
        {
            List<Product> lowOnStockProducts = _inventory.Where(p => p.IsBelowStockThreshold).ToList();
        ...
        private static void ShowFulfilledOrders()
        {
            ...
            foreach (var order in _orders)
            {
                if (!order.Fulfilled && order.OrderFulfilmentDate < DateTime.Now) // fulfil the order
                {
                    ...
                    {
                        Product? selectedProduct = _inventory.Where(p => p.Id == orderItem.ProductId).FirstOrDefault();
                        if (selectedProduct != null)
            ...
            _orders.RemoveAll(o => o.Fulfilled);
